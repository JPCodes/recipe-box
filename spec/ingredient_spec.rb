...
describe(Ingredient) do
  describe('') do
    it('adds quantity to an ingredient') do
      ...
      test_quantity = Ingredients_recipes.create({:recipe_id => test_recipe.id, :ingredient_id => test_ingredient.id, :quantity => "1"})
  ...
  describe('') do
    it('updates correct ingredients_recipes entry') do
      ...
      test_recipe = Recipe.create({:name => 'Steak', :instructions => 'Throw it on the grill', :rating => 8})
      test_ingredient = Ingredient.create({:name => 'lean beef'})
      test_quantity = Ingredients_recipes.create({:recipe_id => test_recipe.id, :ingredient_id => test_ingredient.id, :quantity => nil})
      test_quantity = Ingredients_recipes.where(recipe_id: test_recipe.id)
      ...
      test_quantity2 = Ingredients_recipes.create({:recipe_id => test_recipe.id, :ingredient_id => test_ingredient.id, :quantity => nil})
